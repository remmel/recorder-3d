...
public class BytesToVideoEncoder {
    private static final String TAG = BytesToVideoEncoder.class.getSimpleName();

    private IBitmapToVideoEncoderCallback mCallback;
    private File mOutputFile;
    private Queue<BytesImage> mEncodeQueue = new ConcurrentLinkedQueue();
    ...
    public BytesToVideoEncoder(IBitmapToVideoEncoderCallback callback) {
        mCallback = callback;
    }
    ...
    public void queueFrame(BytesImage image) {
        if (mediaCodec == null || mediaMuxer == null) {
    ...
    private void encode() {
        ...
        while(true) {
            ...
            BytesImage image = mEncodeQueue.poll();
            if (image ==  null) {
                synchronized (mFrameSync) {
            ...
            byte[] byteConvertFrame = image.nv21;
            Log.e(TAG, "byteConvertFrame.length="+byteConvertFrame.length); //153598
